...
Usa termos curtos e claros em português."),
            ...
            try
            {
                ...
                var features = JsonSerializer.Deserialize<List<string>>(response) ?? new List<string>();
...
TAREFA: Verifica se a descrição contém as características solicitadas.
...
foundFeatures: lista das features solicitadas que foram encontradas na descrição"),
...
{description}")
            ...
            try
            {
                ...
                var result = JsonSerializer.Deserialize<MatchResult>(response);
                
                if (result != null)
                {
                    _logger.LogInformation(
                        "[DescriptionAnalyzer] Match score: {Score:P0}, Features encontradas: {Features}",
                        result.MatchScore,
